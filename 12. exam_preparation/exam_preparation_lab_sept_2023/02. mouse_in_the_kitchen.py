ROWS_COUNT, COILS_COUNT = [int(el) for el in input().split(",")]


def is_in_area(row, col):
    return 0 <= row < ROWS_COUNT and 0 <= col < COILS_COUNT
...
while direction != "danger":
    ...
    if not is_in_area(desired_row, desired_col):
        ...
        eaten_cheese_counter += 1
        mouse_position = [desired_row, desired_col]

        if eaten_cheese_counter == total_cheese_count:
